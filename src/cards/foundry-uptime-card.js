...
class FoundryUptimeCard extends HTMLElement {
    ...
    setConfig(config) {
        ...
        this.config.show_footer = this.config.show_footer !== undefined ? this.config.show_footer : true;
        ...
        this._interval = setInterval(() => this._fetchHistory(), this.config.update_interval * 1000); // 60s default
    }
    ...
    set hass(hass) {
        this._hass = hass;
        if (!this._lastFetch || (new Date() - this._lastFetch > this.config.update_interval * 1000)) {
            this._fetchHistory();
        } else {
    ...
    async _fetchHistory() {
        ...
        try {
            // Fetch history
            const history = await this._hass.callApi('GET', `history/period/${isoStart}?filter_entity_id=${entityId}&minimal_response&end_time=${new Date().toISOString()}`);
